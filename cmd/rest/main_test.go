...
func TestRun_FailLogger(t *testing.T) {
	...
	logger.NewLogger = func(level, file string) (logger.ILogger, error) {
		return nil, errors.New("logger init error")
	}

	err := Run()
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "logger init error")
}
...
func TestRun_FailConnectElasticsearch(t *testing.T) {
	...
	err := Run()
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "es error")
}
...
func TestRun_Success(t *testing.T) {
	...
	mockLog := mocks.NewMockILogger(ctrl)
	mockLog.EXPECT().Info("Starting REST server", "port", gomock.Any()).AnyTimes()
	mockLog.EXPECT().Info("Shutting down server gracefully...").AnyTimes()
	mockLog.EXPECT().Info("REST server exiting").AnyTimes()
